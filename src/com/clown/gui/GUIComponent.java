...
import com.clown.io.MouseState;
import com.clown.io.Parentable;
import com.clown.util.ColorPalette;
...
public abstract class GUIComponent extends Component implements Interactable{
	...
	protected void drawMouseEffects(Graphics graphics) {
		...
		if (getMouseState() == MouseState.MOUSEOVER || getMouseState() == MouseState.PRESSED) {
			if (getMouseState() == MouseState.PRESSED)
			...
			for (int i = 0; i < borderWidth / 2; i++) {
				graphics.drawRect(getX() + i, getY() + i, getWidth() - 1 - (i * 2), getHeight() - 1
						- (i * 2));
			}
		}
	}
