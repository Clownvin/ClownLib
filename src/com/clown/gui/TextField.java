package com.clown.gui;

import java.awt.Color;
import java.awt.FontMetrics;
import java.awt.Graphics;

import com.clown.io.Interactable;
import com.clown.io.InteractableManager;
...
public class TextField extends GUIComponent implements Interactable {
	...
	public TextField setText(String text) {
		...
		if (lastFontMetric != null) {
			destClippingWidth = (int) lastFontMetric.getStringBounds(text, null).getWidth();
	...
	protected void checkWidthAndHeight(FontMetrics metric) {
		...
		case LEFT:
			setWidth(getX() + currentClippingWidth + 3 + borderWidth + borderWidth);
			break;
		case CENTER:
	...
	public void draw(Graphics graphics) {
		...
		if (insertPointBlink && hasFocus) {
			...
			graphics.drawLine(getX() + ((getWidth() - metric.stringWidth(getText())) / 2) + currentClippingWidth + 3, getY() + borderWidth, getX() + ((getWidth() - metric.stringWidth(getText())) / 2) + currentClippingWidth + 3, (getY() + getHeight()) - borderWidth);
			graphics.drawLine(getX() + ((getWidth() - metric.stringWidth(getText())) / 2) + currentClippingWidth + 4, getY() + borderWidth, getX() + ((getWidth() - metric.stringWidth(getText())) / 2) + currentClippingWidth + 4, (getY() + getHeight()) - borderWidth);
		}
		graphics.setColor(foreground);
		graphics.setClip(getX() + ((getWidth() - metric.stringWidth(getText())) / 2) - 30, getY(), currentClippingWidth + 30, getHeight());
		graphics.drawString(
				getText().length() >= lastText.length() ? getText() : lastText,
		...
		case RIGHT:
			if (insertPointBlink && hasFocus) {
				graphics.setColor(Color.DARK_GRAY);
				graphics.drawLine(getX() + getWidth() - (borderWidth), getY() + borderWidth, getX() + getWidth() - (borderWidth), (getY() + getHeight()) - borderWidth);
				graphics.drawLine(getX() + getWidth() - (borderWidth)+1, getY() + borderWidth, getX() + getWidth() - (borderWidth)+1, (getY() + getHeight()) - borderWidth);
			}
			graphics.setColor(foreground);
			graphics.setClip(getX() + getWidth() - borderWidth - currentClippingWidth, getY(), currentClippingWidth, getHeight());
			graphics.drawString(
					getText().length() >= lastText.length() ? getText() : lastText,
		...
		case LEFT:
			if (insertPointBlink && hasFocus) {
				graphics.setColor(Color.DARK_GRAY);
				graphics.drawLine(getX() + currentClippingWidth + 3 + borderWidth, getY() + borderWidth, getX() + currentClippingWidth + 3 + borderWidth, (getY() + getHeight()) - borderWidth);
				graphics.drawLine(getX() + currentClippingWidth + 4 + borderWidth, getY() + borderWidth, getX() + currentClippingWidth + 4 + borderWidth, (getY() + getHeight()) - borderWidth);
			}
			graphics.setColor(foreground);
			graphics.setClip(getX() + borderWidth, getY(), currentClippingWidth, getHeight());
			graphics.drawString(
					getText().length() >= lastText.length() ? getText() : lastText,
					getX() + borderWidth,
					(int) (getY() + (metric.getAscent() + metric.getDescent()) + (borderWidth / 2)));
